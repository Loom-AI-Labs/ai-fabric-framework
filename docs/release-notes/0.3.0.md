# AI Fabric 0.3.0 Draft Release Notes

- **Status:** draft for the next public release
- **Release target:** recommended `0.3.0` because this branch contains provider and runtime-baseline breaking changes
- **Current branch reviewed:** `Spirng-AI-Integration` at `92309e0`
- **Compared against:** `origin/main` at `1122bc6`
- **Current POM version:** still `0.2.1`; bump all Maven module versions before publishing
...
The branch diff is intentionally broad: 595 files changed, with 41,187 insertions and 10,382
deletions. The main external migration cost is provider dependency replacement and runtime baseline
alignment.
...
The published version should be the final release version. For this draft, the intended version is:
...
Release engineering must bump the repository POMs from `0.2.1` before publishing.
...
- Spring AI RAG evaluation helpers are intended for tests and release gates, not untrusted end-user
  request paths.
- The repository version still needs a release bump before publishing these notes as a real
  `0.3.0` artifact release.

## Recommended Release Engineer Checklist

1. Bump all Maven modules from `0.2.1` to the chosen release version.
...
6. Update `README.md` latest release link after publishing.
