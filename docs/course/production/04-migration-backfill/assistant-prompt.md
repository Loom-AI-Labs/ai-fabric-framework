...
queue serialization. Add tests for authorization, lifecycle transitions, filters, tenant-safe
retrieval, idempotent rerun, and failure visibility. Extend readiness, HTTP examples, and packaged
smoke.

Do not make vectors the source of truth, accept an arbitrary client entity type, fabricate an exact
skipped count, treat job completion as vector readiness, or require a cloud key. Run `clean verify`
and the packaged ONNX/Lucene smoke.
