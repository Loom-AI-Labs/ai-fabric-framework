...
```bash
git tag -a ai-fabric-framework-v0.5.2 -m "AI Fabric Framework 0.5.2"
git push origin ai-fabric-framework-v0.5.2
```
...
`ai-fabric-framework-v0.5.2`.

Use `curl` before publishing to confirm whether a version already exists:

```bash
curl -s -o /dev/null -w '%{http_code}\n' \
  https://repo1.maven.org/maven2/io/github/loom-ai-labs/ai-fabric-bom/0.5.2/ai-fabric-bom-0.5.2.pom
```
