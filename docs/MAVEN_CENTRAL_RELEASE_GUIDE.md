...
```bash
git tag -a ai-fabric-framework-v0.8.0 -m "AI Fabric Framework 0.8.0"
git push origin ai-fabric-framework-v0.8.0
```
...
branch, bump Maven versions to the next patch version, and publish a new immutable tag such as
`ai-fabric-framework-v<next-patch>`.

Use `curl` before publishing to confirm whether a version already exists:

```bash
curl -s -o /dev/null -w '%{http_code}\n' \
  https://repo1.maven.org/maven2/io/github/loom-ai-labs/ai-fabric-bom/0.8.0/ai-fabric-bom-0.8.0.pom
```
