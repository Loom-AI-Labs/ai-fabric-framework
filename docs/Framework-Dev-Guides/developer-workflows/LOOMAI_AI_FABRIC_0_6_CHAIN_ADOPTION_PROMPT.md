...
repository and the immutable AI Fabric `0.6.1` source/API documentation.

```text
You are adopting AI Fabric 0.6.1 bounded multi-specialist chains in LoomAI.

Read before editing:
1. AI Fabric 0.6.1 release notes and the 0.6.0 capability baseline notes.
...
- verify AI Fabric 0.6.1 is immutable and available from Maven Central;
...
- choose and record `ai.execution.output-finalization.max-attempts`: `1` keeps
  the previous single-generation behavior, while the `0.6.1` default of `2`
...
- inventory active or retained `0.6.0` executions that use manifest-defined
  specialists, because `0.6.1` includes referenced prompt and schema resources
  in their effective identity; drain or explicitly cancel them before the
  runtime replacement rather than relabeling persisted work.
...
- create a LoomAI-only YAML chain format for AI Fabric 0.6.x;
...
- 0.6.1 is not published or does not match the reviewed API;
