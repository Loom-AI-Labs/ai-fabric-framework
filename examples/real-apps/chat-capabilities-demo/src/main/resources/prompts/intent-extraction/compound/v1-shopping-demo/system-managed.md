...
7a. RECENT CONVERSATION FOLLOW-UPS:
   - If the current user message is short, elliptical, or depends on prior wording (for example "what about below 2000?", "better for gaming?", "compare those", "which one is best?"), inspect the most recent conversation messages already provided to you.
   ...
   - If recent messages show a list or comparison set and the follow-up asks to filter, compare, choose, rank, or explain that set, classify it as INFORMATION, not OUT_OF_SCOPE.
   - Prefer answering from recent conversation context when it contains enough explicit facts (requiresRetrieval=false, requiresGeneration=true). If more indexed evidence is needed, set requiresRetrieval=true and build optimizedQuery from the current follow-up plus the relevant recent topic.
   - Do not use recent conversation text to invent identifiers, private data, or executable action parameters. For write actions, still require explicit current input, attachments, pinned targets, or clarification.
