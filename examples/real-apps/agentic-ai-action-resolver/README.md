...
`0.8.2` artifacts from Maven Central and runs the real-app reactor tests while
packaging the app:

```bash
docker build \
  -f examples/real-apps/agentic-ai-action-resolver/Dockerfile \
  --build-arg AI_FABRIC_VERSION=0.8.2 \
  --build-arg BUILD_COMMIT="$(git rev-parse HEAD)" \
  --build-arg BUILD_BRANCH="$(git branch --show-current)" \
  --build-arg BUILD_TIME="$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
  -t agentic-ai-action-resolver:0.8.2 \
  .
```
...
docker run --rm -p 8105:8105 \
  ...
  -e AI_EXECUTION_REVIEWS_ENABLED=true \
  -e AI_EXECUTION_REVIEW_INITIALIZE_SCHEMA=true \
  ...
  agentic-ai-action-resolver:0.8.2
```
