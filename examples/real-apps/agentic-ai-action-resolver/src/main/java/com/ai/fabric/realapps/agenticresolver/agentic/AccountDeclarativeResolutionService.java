...
public class AccountDeclarativeResolutionService {
    ...
    private final AccountSmartResolutionChainInputFactory chainInputFactory;
    private final Clock clock;

    public AccountDeclarativeResolutionService(
        SpecialistChainGateway gateway,
        AgenticResolverSessionService sessions,
        AccountSmartResolutionChainInputFactory chainInputFactory,
        Clock clock
    ) {
        this.gateway = gateway;
        this.sessions = sessions;
        this.chainInputFactory = chainInputFactory;
        this.clock = clock;
    }
    ...
    public AccountSmartResolutionExecutionView submit(
        ...
        SpecialistChainExecutionSnapshot snapshot = gateway.find(
            ...
            "The replayed declarative account-resolution state is unavailable"
        ));
        SpecialistChainExecutionResult result = gateway.findResult(
            handle.executionId(),
            request.trustedExecutionContext()
        ).orElseThrow(() -> new IllegalStateException(
            "The replayed declarative account-resolution result is unavailable"
        ));
    ...
    public AccountSmartResolutionExecutionView status(
        ...
        SpecialistChainExecutionSnapshot snapshot = gateway.find(
            ...
            "Declarative account-resolution execution was not found"
        ));
        ...
        if (snapshot.status().terminal() && result == null) {
            throw new IllegalStateException(
                "The terminal declarative account-resolution result could not be verified"
            );
        }
    ...
    public AccountSmartResolutionExecutionView cancel(
        ...
        if (!gateway.cancel(executionId, context)) {
            throw new IllegalArgumentException(
                "Declarative account-resolution execution is missing or already terminal"
            );
        }
    ...
    private SpecialistChainExecutionRequest<JsonNode> request(
        ...
        return new SpecialistChainExecutionRequest<>(
            ...
            chainInputFactory.create(input),
