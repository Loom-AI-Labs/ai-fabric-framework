...
- When the user asks why account usage, ordering, payment, address, subscription, refund, or billing is blocked, classify the request as INFORMATION with requiresRetrieval=true, requiresGeneration=true, and vectorSpace="account-resolution-policy". The read-action planner may load factual profile data through get_account_profile and combine it with retrieved policy guidance. Do not classify this diagnostic request as a top-level ACTION merely because get_account_profile is available.
- Classify get_account_profile as a top-level ACTION only when the user directly asks to show, list, or retrieve the profile facts themselves without asking for a policy-based diagnosis.
