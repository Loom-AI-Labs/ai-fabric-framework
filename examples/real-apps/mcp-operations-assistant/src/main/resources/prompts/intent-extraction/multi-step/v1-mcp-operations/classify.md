...
- MCP Operations domain override:
  ...
  - Status, health, current revision, and open-incident count -> ACTION with actionHint "get sandbox service status".
  - Recent incidents, symptoms, what happened, why a service appears unhealthy, likely cause, diagnosis, or troubleshooting -> ACTION with actionHint "list recent sandbox incidents".
  - When a diagnosis needs both current state and incident evidence, emit two ACTION intents in order: actionHint "get sandbox service status", then actionHint "list recent sandbox incidents".
  - An explicit restart request -> ACTION with actionHint "restart sandbox service". The governed action flow handles confirmation and revision checks.
  - For these operations, set requiresRetrieval=false and requiresTargetResolution=false. Do not ask for or populate sandboxId, serviceName, expectedRevision, or other backend-owned values.
  - Never assume that wording such as "unhealthy" proves the service is unhealthy. The selected actions provide the facts.
- Highest priority outside the MCP Operations domain: if the USER REQUEST asks about assistant implementation, AI framework infrastructure, internal assistant status, runtime behavior, tool availability, retrieval/vectorization, providers, platform internals, logs, deployments, or secrets, output OUT_OF_SCOPE. Do not classify these requests as INFORMATION and do not set requiresRetrieval=true.
