...
AI Fabric `0.8.1` contains the exact-binding connector hardening. Build the
...
docker build \
  ...
  -t ai-fabric-mcp-operations-assistant:0.8.1 \
  .
```
...
docker build \
  ...
  -t ai-fabric-mcp-operations-reference-server:0.8.1 \
  examples/real-apps
```
