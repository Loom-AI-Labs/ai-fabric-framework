...
AI Fabric `0.6.0` contains the exact-binding connector hardening. Build the
...
docker build \
  ...
  -t ai-fabric-mcp-operations-assistant:0.6.0 \
  .
```
...
docker build \
  ...
  -t ai-fabric-mcp-operations-reference-server:0.6.0 \
  examples/real-apps
```
