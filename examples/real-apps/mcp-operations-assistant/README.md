...
AI Fabric `0.8.0` contains the exact-binding connector hardening. Build the
...
docker build \
  ...
  -t ai-fabric-mcp-operations-assistant:0.8.0 \
  .
```
...
docker build \
  ...
  -t ai-fabric-mcp-operations-reference-server:0.8.0 \
  examples/real-apps
```
