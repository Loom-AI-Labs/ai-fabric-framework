...
import ai.fabric.config.SmartSuggestionsProperties;
...
import ai.fabric.intent.orchestration.pipeline.steps.AccessControlStep;
import ai.fabric.intent.orchestration.pipeline.steps.HistoryPersistenceStep;
import ai.fabric.intent.orchestration.pipeline.steps.IntentExtractionStep;
import ai.fabric.intent.orchestration.pipeline.steps.IntentHandlingStep;
import ai.fabric.intent.orchestration.pipeline.steps.MetadataBuildingStep;
import ai.fabric.intent.orchestration.pipeline.steps.OrchestrationResultNormalizationStep;
import ai.fabric.intent.orchestration.pipeline.steps.ResponseSanitizationStep;
import ai.fabric.intent.orchestration.pipeline.steps.SecurityAnalysisStep;
import ai.fabric.intent.orchestration.pipeline.steps.SmartSuggestionsStep;
import ai.fabric.intent.orchestration.pipeline.steps.VectorSpaceResolutionStep;
...
class RAGOrchestratorTest {
    ...
    private Pipeline createPipeline() {
        var normalizationProperties = new ai.fabric.config.OrchestrationResultNormalizationProperties();
        ...
        var vectorSpaceRoutingProperties = new ai.fabric.config.VectorSpaceRoutingProperties();
        var rankBasedMerger = new ai.fabric.intent.vectorspace.RankBasedMerger();
        var vectorSpaceRouter = mock(ai.fabric.intent.vectorspace.VectorSpaceRouter.class);
        lenient().when(vectorSpaceRouter.route(any(), anyString())).thenReturn(
            ai.fabric.intent.vectorspace.RoutingResult.builder()
                .success(true)
                .vectorSpace("policies")
                .strategy(ai.fabric.intent.vectorspace.RoutingStrategy.HEURISTIC)
                .confidence(0.5d)
                .rationale("test default")
                .build()
        );

        ObjectProvider<ai.fabric.intent.extraction.ProgressiveIntentExtractionEngine> progressiveEngineProvider =
        ...
        PromptRenderer promptRenderer = new PromptRenderer();
	        ...
	        List<PipelineStep> steps = List.of(
	            ...
	            new IntentHandlingStep(actionHandlerRegistry, ragProviderProvider, aiCoreService, aiServiceConfig, advancedRagProvider,
	                ...
	                new ai.fabric.config.RelationshipQueryPostActionGenerationProperties(),
	                new ai.fabric.config.PostActionGenerationProperties(),
    ...
    void shouldExecuteActionIntent() {
        ...
        OrchestrationResult result = orchestrator.orchestrate("Cancel my plan", ai.fabric.intent.orchestration.OrchestrationContext.forUser("user-1"));
    ...
    void shouldReturnErrorWhenHandlerMissing() {
        ...
        OrchestrationResult result = orchestrator.orchestrate("Do something", ai.fabric.intent.orchestration.OrchestrationContext.forUser("user"));
    ...
    void shouldHandleActionValidationFailure() {
        ...
        OrchestrationResult result = orchestrator.orchestrate("Cancel", ai.fabric.intent.orchestration.OrchestrationContext.forUser("user"));
    ...
    void shouldProcessInformationIntentViaRag() {
        ...
        OrchestrationResult result = orchestrator.orchestrate("What is your refund policy?", ai.fabric.intent.orchestration.OrchestrationContext.forUser("user"));
    ...
    void shouldRouteInformationIntentToGenerationWhenRequested() {
        ...
        OrchestrationResult result = orchestrator.orchestrate("Recommend products under $100", ai.fabric.intent.orchestration.OrchestrationContext.forUser("user"));
    ...
    void shouldUseAdvancedRagWhenLlmRequestsIt_andFallbackToOrchestratorGenerationWhenProviderOmitsResponse() {
        ...
        OrchestrationResult result = orchestrator.orchestrate("What should I buy for commuting?", ai.fabric.intent.orchestration.OrchestrationContext.forUser("user"));
    ...
    void shouldNotUseAdvancedRagWhenLlmDeclinesEvenIfHeuristicsWouldMatch() {
        ...
        OrchestrationResult result = orchestrator.orchestrate("Recommend audio gear", ai.fabric.intent.orchestration.OrchestrationContext.forUser("user"));
    ...
    void shouldHandleOutOfScopeIntent() {
        ...
        OrchestrationResult result = orchestrator.orchestrate("Build me a spaceship", ai.fabric.intent.orchestration.OrchestrationContext.forUser("user"));
    ...
    void shouldUseSafeDefaultForOutOfScopeWhenModelOnlyProvidesDirectAnswer() {
        ...
        OrchestrationResult result = orchestrator.orchestrate("Give me legal advice", ai.fabric.intent.orchestration.OrchestrationContext.forUser("user"));
    ...
    void shouldHandleCompoundIntents() {
        ...
        OrchestrationResult result = orchestrator.orchestrate("Cancel and explain refund", ai.fabric.intent.orchestration.OrchestrationContext.forUser("user"));
    ...
    void shouldIncludeNextStepRecommendations() {
        ...
        OrchestrationResult result = orchestrator.orchestrate("Update my payment method", ai.fabric.intent.orchestration.OrchestrationContext.forUser("user"));
